...
class DeclListNode extends ASTnode {
    ...
    public void codeGen() {
        ...
        try {
            DeclNode myDecl;
            while (it.hasNext()) {
                myDecl = (DeclNode)it.next();
                if(myDecl instanceof VarDeclNode) {
                    ((VarDeclNode)myDecl).codeGen();
                }
                else if(myDecl instanceof FnDeclNode) {
                    ((FnDeclNode)myDecl).codeGen();
                }
            }
        } catch (NoSuchElementException ex) {
            System.err.println("unexpected NoSuchElementException in DeclListNode.print");
...
class FnBodyNode extends ASTnode {
     ...
     * code generation
     */
    public void codeGen(String l) {
        ProgramNode.isGlobal = false;
        myStmtList.codeGen(l);
        ProgramNode.isGlobal = true;
    }
...
class VarDeclNode extends DeclNode {
    ...
    public Sym nameAnalysis(SymTable symTab, SymTable globalTab) {
        ...
        if(!ProgramNode.isGlobal) {
            sym.setOffset(FnDeclNode.offset);
            FnDeclNode.offset += -4;
            FnDeclNode.sizeOfLocals += 4;
        }
        return sym;
    }    
    ...
    public void codeGen() {
        if(myId.isGlobal()) {
            ...
            Codegen.generateLabeled("_" + myId.name(), ".space 4", "Global variable");
        }
    }
...
class FnDeclNode extends DeclNode {
    ...
    public Sym nameAnalysis(SymTable symTab) {
        ...
        sym.setSizeOfLocals(FnDeclNode.sizeOfLocals);
        sym.setSizeOfParams(typeList.size()*4);
        ProgramNode.isGlobal = true;
        return null;
    } 
    ...
    public void codeGen() {
        if(myId.isMain()) {
            ...
            Codegen.generate("addu", Codegen.FP, Codegen.SP, 8 + ((FnSym)myId.sym()).getSizeOfParams());
            //Codegen.generate("subu", Codegen.SP, Codegen.SP, ((FnSym)myId.sym()).getSizeOfLocals());
            myBody.codeGen("_main_Exit");
            Sym s = myId.sym();
            Codegen.genLabel("_main_Exit");
            Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, -((FnSym)s).getSizeOfParams());
            Codegen.generate("move", Codegen.T0, Codegen.FP);
            Codegen.generateIndexed("lw", Codegen.FP, Codegen.FP, -(((FnSym)s).getSizeOfParams()+4));
    ...
    public static int sizeOfLocals;
}
...
abstract class TypeNode extends ASTnode {
    ...
    // no code generation for TypeNode
}
...
abstract class StmtNode extends ASTnode {
    ...
    abstract public void codeGen(String l);
}
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen(String l) {
        if(myExp instanceof IdNode){
            Codegen.generate("lw", Codegen.T0, Codegen.SP, ((IdNode)myExp).getOffset());
            //Codegen.genPop(Codegen.T0, 4);
            Codegen.generate("addu", Codegen.T0, Codegen.T0, "1");
            //Codegen.genPush(Codegen.T0, 4);
            Codegen.generate("sw", Codegen.T0, Codegen.SP, ((IdNode)myExp).getOffset());
        }
    }
...
class ReadStmtNode extends StmtNode {
    ...
    public void codeGen(String l) {
        if(myExp instanceof IdNode) {
            ...
            if(((IdNode)myExp).isGlobal())
                Codegen.generate("sw", Codegen.V0, "_"+((IdNode)myExp).name());
            else
                Codegen.generateIndexed("sw", Codegen.V0, Codegen.SP, ((IdNode)myExp).sym().getOffset());
        }
    }
...
class IdNode extends ExpNode {
    ...
    /**
     * return true if it is a global variable
     */
    ...
    public void codeGen() {
        ...
        if(mySym.isGlobal()) {
            ...
            Codegen.generateIndexed("lw", Codegen.T0, Codegen.SP, mySym.getOffset());
            Codegen.genPush(Codegen.T0);
        }
    }
    ...
    public void codeGenAddr() {
        Codegen.generateWithComment("", "IdNode Address");
        if(mySym.isGlobal()) {
...
class AssignNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        if(myLhs instanceof IdNode) {
            if(((IdNode)myLhs).isGlobal()) {
                myExp.codeGen();
                Codegen.generate("move", Codegen.T0, Codegen.SP);
                Codegen.generate("sw", Codegen.T0, "_"+((IdNode)myLhs).name());
            }
            else {
                myExp.codeGen();
                Codegen.generate("move", Codegen.T0, Codegen.SP);
                Codegen.generateIndexed("sw", Codegen.T0, Codegen.SP, ((IdNode)myLhs).getOffset());
            }
        }
    }
